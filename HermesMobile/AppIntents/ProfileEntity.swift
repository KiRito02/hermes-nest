...
enum ProfileEntityProvider {
    ...
    ///
    /// Runs out-of-process (App Intents), where the foreground-only
    /// `CustomHeaderStore.shared` snapshot is empty. So we read the server's
    ...
    /// blob) without mutating the Keychain. This stays read-only because it runs
    /// in the App Intents process.
