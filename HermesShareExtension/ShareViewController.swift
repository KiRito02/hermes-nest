...
final class ShareViewController: UIViewController {
    ...
    private func configureView() {
        ...
        statusLabel.text = "Opening Hermes Nest..."
    ...
    private func saveDraftAndOpenHermes() async {
        ...
        guard !draft.isEmpty || !input.attachments.isEmpty else {
            showStatus("Hermes Nest accepts text, URLs, images, PDFs, and files up to 20 MB.")
            completeRequest(after: 0.8)
            return
        }

        guard let directory = HermesShareDraft.containerURL() else {
            showStatus("Could not access Hermes Nest storage.")
            completeRequest(after: 0.8)
            return
        }
    ...
    private func openHermesViaWorkaround(_ url: URL) {
        ...
        showStatus("Shared content saved. Open Hermes Nest manually.")
